...
#You'll have an if statement that places the value from one column into one list, then the second column in the next list
...
#For transposing, we need a different kind of loop. We are looping through each item in the list and we create our own iterator for the column
#Set each value to have row=1 and add the column number after each value is copied to the new sheet
#Repeat for the second list, but row=2
col_count = 1
col_count_2 = 1
for item in col_1:
...
for item in col_2:
    ...
    col_count_2 += 1
os.chdir(r'')#Choose the folder you wish
